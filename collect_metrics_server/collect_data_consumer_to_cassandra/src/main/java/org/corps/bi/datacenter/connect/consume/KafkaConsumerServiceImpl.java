...
public class KafkaConsumerServiceImpl implements ConsumerService{
	...
	private final CassandraClusterSessionManager cassandraClusterSessionManager;
	...
	public KafkaConsumerServiceImpl() {
		...
		this.threadPoolExecutor = new ThreadPoolExecutor(Constant.getInstance().getMetricConsumeThreadCoreSize(),		//指的是保留的线程池大小
				...
				new BISystemThreadFactory("datacener-metric-consume-to-cassandra"),
				new ThreadPoolExecutor.CallerRunsPolicy() //直接放弃当前任务
		);
		this.cassandraClusterSessionManager=this.initCassandraCluster();
		
		this.init();
	}
	
	private CassandraClusterSessionManager initCassandraCluster() {
		CassandraClusterSessionManager cassandraManager=new CassandraClusterSessionManager(Constant.getInstance().getCassandraConfig());
	...
	private MetricTopicConsumer doConsume(ConnectMetricConfig connectMetricConfig) throws Exception {
		...
		return new MetricTopicConsumer(connectMetricConfig,metricProperties,this.cassandraClusterSessionManager.getDefaultSession());
	}
	...
	public void shutdown() {
		...
		try {
			...
			this.cassandraClusterSessionManager.close();
